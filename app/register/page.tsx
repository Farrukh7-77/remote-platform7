...
function RegisterForm() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center py-12">
      <div className="max-w-md w-full mx-auto px-4">
        <div className="bg-white rounded-xl shadow-md p-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setRole("job_seeker")}
                  className={`py-2 px-4 rounded-lg border transition cursor-pointer ${role === "job_seeker" ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"}`}>
                  👤 Job Seeker
                </button>
                <button type="button" onClick={() => setRole("employer")}
                  className={`py-2 px-4 rounded-lg border transition cursor-pointer ${role === "employer" ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"}`}>
                  🏢 Employer
                </button>
              </div>
            </div>
          ...
          <p className="text-center text-sm text-gray-600 mt-4">
            ...
            <button onClick={() => setIsSignInModalOpen(true)} className="text-blue-600 hover:underline cursor-pointer">
              Sign In
            </button>
          </p>
        </div>
      ...
      <AuthModal isOpen={isSignInModalOpen} onClose={() => setIsSignInModalOpen(false)} />
    </div>
  );
}
