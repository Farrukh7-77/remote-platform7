...
// SVG Icons - will be colored blue in FilterCard
...
>
      ...
      <section className="relative overflow-hidden pt-5 pb-5 px-4 bg-cover bg-no-repeat md:bg-center bg-top" style={{ backgroundImage: "url('/hero-bg.png')", backgroundPosition: "center 48%" }}>
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-3 inline-flex items-center gap-2 flex-wrap justify-center relative" style={{ top: "-17px" }}>
            ...
            <img 
              src="/job-icon.png" 
              alt="job icon" 
              className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 object-contain inline-block relative"
            ...
            <div className="relative group">
              <div className="relative flex items-center bg-[#0f172a]/60 backdrop-blur-sm border border-white/15 rounded-lg overflow-hidden focus-within:border-blue-500/40 transition-all duration-300">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-3 bg-transparent focus:outline-none text-white placeholder-gray-400 text-sm"
                />
                <button className="bg-gradient-to-r from-blue-700 to-blue-600 text-white font-semibold py-2 px-5 m-1 rounded-md transition-all duration-200 hover:scale-[1.02] flex items-center gap-1 text-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row gap-6">
          {/* FILTER SIDEBAR - Glassmorphism with ml-7 for right shift */}
          <div className={`${showFilters ? "block animate-slide-down" : "hidden"} md:block md:w-72 ml-7 -mt-4`}>
            <div className="glass-card p-5 transition-all duration-300 sticky top-24">
              <div className="relative flex justify-between items-center mb-5">
                <h3 className="font-bold text-white text-lg">Filters</h3>
                <button 
                  onClick={resetFilters} 
                  className="flex items-center gap-1 text-sm text-red-500 hover:text-red-400 transition-colors cursor-pointer"
                >
                  <ResetIcon />
                  Reset all
                </button>
          ...
          <div className="flex-1 max-w-4xl mx-auto">
            {/* Tabs and Sort Row - Tabs on left, Sort on right */}
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  All Jobs <span className="ml-1 text-xs opacity-80">({totalJobs})</span>
                </button>
                <button
                ...
                >
                  Saved <span className="ml-1 text-xs opacity-80">({savedCount})</span>
                </button>
                <button
                ...
                >
                  Applied <span className="ml-1 text-xs opacity-80">({appliedCount})</span>
                </button>
              </div>
              ...
              <div className="sort-dropdown relative">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 glass-card text-sm text-gray-300 hover:text-white transition-colors cursor-pointer"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
                  </svg>
                  <span>Sort: <span className="text-blue-400">
                    {SORT_OPTIONS.find(o => o.value === sortBy)?.label}
                  </span></span>
                  <svg className={`w-4 h-4 transition-transform duration-200 ${sortOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                
                {sortOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#0f172a] rounded-xl border border-white/10 py-1 z-20 animate-fade-in-up">
            ...
            {sortedJobs.length === 0 && (
              <div className="glass-card p-12 text-center">
                ...
                <button onClick={resetFilters} className="mt-4 text-blue-400 hover:text-blue-300 text-sm cursor-pointer">
                  Clear all filters
                </button>
              </div>
            )}
...
// FilterCard component with 3D effect and larger size
function FilterCard({
  title,
  icon,
  open,
  ...
  return (
    <div className="border border-white/15 rounded-xl overflow-hidden shadow-[0_2px_6px_rgba(0,0,0,0.35)] transition-all duration-200 hover:-translate-y-0.5">
      <button 
        onClick={onToggle} 
        className="w-full flex justify-between items-center text-gray-300 cursor-pointer px-4 py-4 transition-colors duration-200 hover:text-white bg-[#0a0f1a]"
      >
        <span className="flex items-center gap-2.5 text-sm font-medium">
          {/* Icon - bright blue (always) */}
      ...
      {open && (
        <div className="px-4 pb-4 space-y-1.5 border-t border-white/10 pt-3 animate-slide-down bg-[#0f172a]">
          {!isSalary && items && selected && onChange && items.map((item) => (
            <label key={item} className="flex items-center gap-2 text-sm text-gray-400 cursor-pointer hover:text-blue-400 transition-colors duration-200 py-0.5">
