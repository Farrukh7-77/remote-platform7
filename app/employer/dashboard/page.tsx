...
export default function EmployerDashboard() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      const myJobs = allJobs.filter((job: any) => job.postedBy === user.email);
      setJobs(myJobs);
    }
  }, [user, loading, router]);
            ...
            {jobs.map((job) => (
              <div key={job.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition">
                ...
                <div className="mt-3 flex gap-3">
                  ...
                  <Link href={`/employer/jobs/${job.id}/applications`} className="text-sm text-green-600 hover:underline">Applications</Link>
                </div>
              </div>
            ))}
          </div>
